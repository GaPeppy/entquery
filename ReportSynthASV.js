...
//
//
async function GetEntityLoop() {
  const METHODNAME='GetEntityLoop'
...
async function main() {
  ...
  aApps.forEach((val,idx) => {
    slog = [val.account.id,'"'+val.entityType+'"',val.monitorId,'"' + val.name + '"','"' + val.asv + '"']
